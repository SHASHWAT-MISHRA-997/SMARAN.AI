...
const ExtensionsHub = ({ isOpen = true, onClose, embedded = false }) => {
  ...
  return (
    <div className={embedded
      ? 'h-full min-h-0 w-full overflow-hidden bg-zinc-50 dark:bg-zinc-950 p-3 sm:p-6 text-zinc-900 dark:text-zinc-100 transition-colors'
      : 'fixed inset-0 z-[85] flex items-center justify-center bg-black/60 dark:bg-black/85 p-4 backdrop-blur-sm'}>
      <div className={embedded
        ? 'mx-auto flex h-full w-full max-w-6xl overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-2xl'
        : 'flex h-[min(48rem,94vh)] w-full max-w-5xl overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-2xl'}>

        <div className="flex min-w-0 flex-1 flex-col">
          <header className="flex items-center gap-2 border-b border-zinc-200 dark:border-zinc-800 px-3 sm:px-5 py-3 bg-zinc-50 dark:bg-zinc-950">
            <div className="flex min-w-0 flex-1 items-center gap-1.5 sm:gap-2">
              <button type="button" onClick={load} title="Refresh runtime state"
                className="shrink-0 rounded-lg p-2 text-zinc-500 dark:text-zinc-400 transition hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white border border-zinc-200 dark:border-zinc-800 cursor-pointer">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-xl bg-indigo-600 px-2.5 sm:px-3.5 py-1.5 text-[11px] sm:text-xs font-black text-white transition hover:bg-indigo-500 shadow-md shadow-indigo-600/30 cursor-pointer"
              >
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1 rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-2.5 sm:px-3 py-1.5 text-xs font-bold text-zinc-800 dark:text-zinc-200 transition hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer"
                >
                  <span>Add</span> <ChevronDown className="h-3.5 w-3.5" />
                </button>
                {showAddMenu && (
                  <div className="absolute right-0 top-full z-30 mt-2 w-56 overflow-hidden rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 p-1.5 shadow-2xl backdrop-blur-md">
                    <button type="button" onClick={() => { setAdding('mcp'); setShowAddMenu(false); }} className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-semibold text-zinc-700 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer">
                      <Wrench className="h-3.5 w-3.5 text-amber-500" /> Add MCP Server
                    </button>
                    <button type="button" onClick={() => { setShowCatalogue(true); setShowAddMenu(false); }} className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-semibold text-zinc-700 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer">
                      <Blocks className="h-3.5 w-3.5 text-cyan-500" /> Browse MCP servers
                    </button>
                    <button type="button" onClick={() => { setAdding('repo'); setShowAddMenu(false); }} className="w-full flex items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-semibold text-zinc-700 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer">
                      <FolderGit2 className="h-3.5 w-3.5 text-emerald-500" /> Install from Git Repository
                    </button>
                  </div>
                )}
              </div>
            ...
            <button
              ...
              className="flex shrink-0 items-center gap-1.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-900/80 px-2.5 sm:px-3 py-2 text-xs font-bold text-zinc-700 dark:text-zinc-300 transition hover:bg-zinc-200 dark:hover:bg-zinc-800 hover:text-zinc-950 dark:hover:text-white cursor-pointer"
            >
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            <div className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-10 sm:py-8">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-white">{title}</h2>
                  <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">{subtitle}</p>
                </div>
              </div>

              {noBackend() && (
                <div className="mt-6 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-700 dark:text-amber-200">
              ...
              {loadError && <p role="alert" className="mt-4 text-sm text-rose-500 dark:text-rose-300">{loadError} Use Refresh to try again.</p>}
              <div className="relative mt-6 min-w-0 overflow-hidden">
                ...
                <input
                  ...
                  className="w-full rounded-full border border-zinc-300 dark:border-zinc-700/80 bg-zinc-50 dark:bg-zinc-900/80 py-3 pl-11 pr-4 text-sm text-zinc-900 dark:text-zinc-100 outline-none placeholder:text-zinc-400 dark:placeholder:text-zinc-500 focus:border-indigo-500"
                />
              </div>

              {(
                <div className="mt-6 flex flex-wrap gap-2 border-b border-zinc-200 dark:border-zinc-800 pb-4">
                  {MANAGE_FILTERS.map(({ id, label, icon: Icon }) => (
                    <button
                      ...
                      className={`flex items-center gap-2 rounded-xl px-3.5 py-2 text-xs font-bold transition cursor-pointer ${
                        section === id
                          ? 'bg-white dark:bg-zinc-800 font-bold text-zinc-950 dark:text-white border border-zinc-300 dark:border-zinc-700 shadow-sm'
                    ...
                    >
                      ...
                      <span className={`ml-1 rounded-md px-1.5 py-0.5 text-[10px] ${
                        section === id ? 'bg-indigo-600 text-white' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400'
                      }`}>
                        {counts[id] || 0}
                      </span>
                    </button>
                ...
                <div className="divide-y divide-zinc-100 dark:divide-zinc-800/80">
                  {visible.map((row) => {
                    ...
                    return (
                      <div
                        key={row.id || row.name}
                        className="group flex flex-col sm:flex-row sm:items-center justify-between gap-3 py-4 transition hover:bg-zinc-100/60 dark:hover:bg-zinc-900/30 rounded-xl px-2.5"
                      >
                        <button
                          type="button"
                          onClick={() => setDetail(row)}
                          className="min-w-0 flex flex-1 items-start gap-3.5 text-left cursor-pointer"
                        >
                          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-zinc-200 dark:border-zinc-700/80 bg-zinc-100 dark:bg-zinc-900 text-indigo-600 dark:text-indigo-400 shadow-inner">
                            {row.type === 'skill' ? <Sparkles className="h-5 w-5 text-amber-500" /> :
                             row.type === 'mcp' ? <Wrench className="h-5 w-5 text-indigo-500 dark:text-indigo-400" /> :
                             row.type === 'connector' ? <Plug className="h-5 w-5 text-cyan-500" /> :
                             <Blocks className="h-5 w-5 text-emerald-500" />}
                          </span>
                          <span className="min-w-0 flex-1">
                            <span className="flex items-center gap-2">
                              <span className="font-bold text-zinc-900 dark:text-zinc-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-300 text-sm">
                                {row.name}
                              </span>
                              {row.is_custom && (
                                <span className="rounded-md bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-700/50 px-1.5 py-0.5 text-[9px] font-bold text-indigo-600 dark:text-indigo-300">
                                  Custom
                                </span>
                              )}
                            </span>
                            <span className="mt-0.5 line-clamp-2 block text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">
                              {row.description || row.target || '—'}
                            </span>
                            {row.runtime_status === 'setup_required' && row.status_detail && (
                              <span className="mt-1.5 block text-[11px] leading-relaxed text-amber-600 dark:text-amber-400/90">
                                {row.status_detail}
                              </span>
                            )}
                            {row.capabilities?.length > 0 && (
                              <span className="mt-2 flex flex-wrap gap-1">
                                {row.capabilities.slice(0, 4).map((cap) => (
                                  <span key={typeof cap === 'string' ? cap : cap.name} className="rounded-md bg-zinc-100 dark:bg-zinc-800/80 border border-zinc-200 dark:border-zinc-700/50 px-2 py-0.5 font-mono text-[10px] text-zinc-700 dark:text-zinc-300">
                        ...
                        <div className="flex items-center gap-3 self-end sm:self-center">
                          {row.type === 'skill' && row.is_custom && row.instructions && (
                            <button type="button" className="text-xs text-indigo-600 dark:text-cyan-300 hover:underline cursor-pointer"
                              onClick={() => {
                          ...
                          {row.is_custom ? (
                            <button
                              ...
                              className="rounded-lg p-2 text-zinc-500 transition hover:bg-rose-500/10 hover:text-rose-500 border border-zinc-200 dark:border-zinc-800 cursor-pointer"
                            ...
                            <button
                              ...
                              className={`rounded-xl px-3.5 py-1.5 text-xs font-black transition cursor-pointer ${
                                on
                                  ? 'border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-800'
          ...
          <footer className="flex items-center justify-between border-t border-zinc-200 dark:border-zinc-800 px-5 py-3 text-[11px] text-zinc-500 bg-zinc-50 dark:bg-zinc-950">
            <span>
              {running} of {visible.length} running
            </span>
            {(() => {
              if (loadError) return <span className="font-semibold text-rose-500 dark:text-rose-400">● unavailable</span>;
              ...
              if (broken) return <span className="font-semibold text-rose-500 dark:text-rose-400">● {broken} failed</span>;
              if (waiting) return <span className="font-semibold text-amber-500 dark:text-amber-400">● {waiting} need setup</span>;
              if (ready) return <span className="font-semibold text-zinc-500 dark:text-zinc-400">● {ready} ready</span>;
              if (running !== visible.length) return <span>{visible.length - running} not running</span>;
              return <span className="font-semibold text-emerald-500 dark:text-emerald-400">● all running</span>;
            })()}
          </footer>
        </div>
      </div>
      ...
      {showCatalogue && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/60 dark:bg-black/70 p-3 backdrop-blur-sm">
          <div className="w-full max-w-2xl max-h-[85vh] overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-950 shadow-2xl flex flex-col text-zinc-900 dark:text-zinc-100">
            <div className="flex items-start justify-between gap-3 border-b border-zinc-200 dark:border-zinc-800 p-4">
              <div>
                <h3 className="text-sm font-black text-zinc-900 dark:text-white">MCP servers</h3>
                <p className="mt-0.5 text-[11px] leading-5 text-zinc-600 dark:text-zinc-400">
              ...
              <button type="button" onClick={() => { setShowCatalogue(false); setServerNote(''); }} className="p-1.5 text-zinc-400 hover:text-zinc-900 dark:hover:text-white cursor-pointer">
                <X className="h-4 w-4" />
              </button>
            </div>

            {serverNote && (
              <p className="mx-4 mt-3 rounded-lg border border-indigo-200 dark:border-indigo-800/60 bg-indigo-50 dark:bg-indigo-950/40 px-3 py-2 text-[11px] text-indigo-700 dark:text-indigo-200">
                {serverNote}
              </p>
            )}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2.5">
              ...
              {catalogue.map((entry) => (
                <div key={entry.name} className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/60 p-3.5 shadow-xs">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <span className="block text-xs font-extrabold text-zinc-900 dark:text-white">{entry.title}</span>
                      <span className="mt-0.5 block text-[11px] leading-5 text-zinc-600 dark:text-zinc-400">{entry.description}</span>
                      <code className="mt-1.5 block truncate text-[10px] text-zinc-500 font-mono">{entry.target}</code>
                      {entry.needs && (
                        <span className="mt-1 block text-[10px] font-semibold text-amber-600 dark:text-amber-400">Needs {entry.needs}</span>
                      )}
                      {entry.verified && (
                        <span className="mt-1 block text-[10px] text-emerald-600 dark:text-emerald-400">{entry.verified}</span>
                      )}
                    </div>
                    <button
                      type="button"
                      ...
                      className="shrink-0 rounded-lg bg-indigo-600 px-3 py-1.5 text-[11px] font-black text-white transition hover:bg-indigo-500 disabled:opacity-40 cursor-pointer shadow-xs"
                    >
...
const CreateStudio = ({ onClose, onCreated }) => {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/60 dark:bg-black/80 p-4 backdrop-blur-sm" onClick={onClose}>
      <form
        onSubmit={submit}
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-2xl overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-950 shadow-2xl flex flex-col max-h-[90vh] text-zinc-900 dark:text-zinc-100"
      >
        <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 px-6 py-4 bg-zinc-50 dark:bg-zinc-900/60">
          <div>
            <h3 className="text-base font-extrabold text-zinc-900 dark:text-white flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-indigo-600 dark:text-indigo-400" /> Create Custom Skill / MCP Server
            </h3>
            <p className="text-xs text-zinc-600 dark:text-zinc-400 mt-0.5">
              Build your own custom agent skills, MCP tools, and plugin connectors.
            </p>
          </div>
          <button type="button" onClick={onClose} className="rounded-lg p-1.5 text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white cursor-pointer">
            <X className="h-4 w-4" />
          </button>
        </div>

        {/* Extension Type Selector */}
        <div className="px-6 pt-4 pb-2 flex gap-2 border-b border-zinc-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-950">
          {[
            ...
            return (
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-bold transition border cursor-pointer ${
                  extType === tab.id
                    ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-200 shadow-sm'
                    : 'border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50 text-zinc-600 dark:text-zinc-400 hover:text-zinc-950 dark:hover:text-zinc-200'
                }`}
              >
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-1">
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 px-3.5 py-2.5 text-sm text-zinc-900 dark:text-zinc-100 outline-none focus:border-indigo-500"
            />
          </div>
            ...
            <>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 p-3 text-sm text-zinc-900 dark:text-zinc-100 outline-none focus:border-indigo-500 font-mono"
                />
              </div>
            </>
          )}
          ...
          {extType === 'mcp' && (
            <>
              <div>
                <label className="block text-xs font-bold text-zinc-700 dark:text-zinc-300 mb-1.5">
                  Command or SSE Target URL <span className="text-indigo-500 dark:text-indigo-400">*</span>
                </label>
                <input
                  required
                  value={targetCmd}
                  ...
                  className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 px-3.5 py-2.5 text-sm font-mono text-indigo-600 dark:text-indigo-300 outline-none focus:border-indigo-500"
                />
              </div>
          ...
          {error && (
            <p className="rounded-xl border border-rose-300 dark:border-rose-800/60 bg-rose-50 dark:bg-rose-950/40 p-3 text-xs text-rose-600 dark:text-rose-300">
              {error}
            </p>
          )}
        </div>

        <div className="flex items-center justify-end gap-3 border-t border-zinc-200 dark:border-zinc-800 px-6 py-4 bg-zinc-50 dark:bg-zinc-900/40">
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl px-4 py-2 text-xs font-bold text-zinc-600 dark:text-zinc-400 hover:text-zinc-950 dark:hover:text-white cursor-pointer"
          >
            Cancel
          </button>
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-2 text-xs font-bold text-white hover:bg-indigo-500 disabled:opacity-40 transition shadow-lg shadow-indigo-600/30 cursor-pointer"
          >
...
const DetailPanel = ({ row, onClose }) => {
  const state = STATE[row.runtime_status] || STATE.not_connected;
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/60 dark:bg-black/70 p-4" onClick={onClose}>
      <div
        className="w-full max-w-lg overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-2xl text-zinc-900 dark:text-zinc-100"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-start gap-3 border-b border-zinc-200 dark:border-zinc-800 px-5 py-4">
          <div className="min-w-0 flex-1">
            <h3 className="font-black text-zinc-900 dark:text-white">{row.name}</h3>
            <p className="mt-0.5 text-[11px] text-zinc-500">{row.description || row.target || '—'}</p>
          </div>
          <button type="button" onClick={onClose} aria-label="Close"
                  className="rounded-lg p-1.5 text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white cursor-pointer">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="space-y-3 p-5">
          ...
          {row.status_detail && (
            <p className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-black/40 px-3 py-2.5 text-[11px] leading-relaxed text-zinc-700 dark:text-zinc-400 font-mono">
              {row.status_detail}
            </p>
          )}
              ...
              <ul className="flex flex-wrap gap-1.5">
                {row.capabilities.map((c) => (
                  <li key={typeof c === 'string' ? c : c.name}
                      className="rounded-lg bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-2.5 py-1 font-mono text-[11px] text-zinc-800 dark:text-zinc-300">
...
const AddFromRepo = ({ onClose, onSaved }) => {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/60 dark:bg-black/70 p-4" onClick={onClose}>
      <form
        onSubmit={submit}
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-lg rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-5 shadow-2xl text-zinc-900 dark:text-zinc-100"
      >
        <h3 className="text-sm font-black text-zinc-900 dark:text-white">Add from a repository</h3>
        <p className="mt-1 text-[11px] leading-relaxed text-zinc-500">
          The repository is cloned and registered into SMARAN.AI.
        </p>

        <label className="mt-4 block text-[11px] font-bold uppercase tracking-wider text-zinc-600 dark:text-zinc-400">
          Repository URL
        </label>
        <input
          autoFocus
          ...
          className="mt-1.5 w-full rounded-lg border border-zinc-300 dark:border-zinc-800 bg-zinc-50 dark:bg-black px-3 py-2 text-sm
                     text-zinc-900 dark:text-white outline-none focus:border-indigo-500 font-mono text-xs"
        />

        {error && (
          <p className="mt-3 rounded-lg border border-rose-300 dark:border-rose-900/60 bg-rose-50 dark:bg-rose-950/40 px-3 py-2 text-[11px] text-rose-600 dark:text-rose-300">
            {error}
          </p>
        )}
        {done && (
          <p className="mt-3 rounded-lg border border-emerald-300 dark:border-emerald-900/60 bg-emerald-50 dark:bg-emerald-950/40 px-3 py-2 text-[11px] text-emerald-700 dark:text-emerald-300">
            {done}
          </p>
        )}
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg px-3 py-1.5 text-xs font-bold text-zinc-600 dark:text-zinc-400 hover:text-zinc-950 dark:hover:text-white cursor-pointer"
          >
            Cancel
          </button>
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3.5 py-1.5 text-xs font-black
                       text-white transition hover:bg-indigo-500 disabled:opacity-40 cursor-pointer shadow-xs"
          >
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/60 dark:bg-black/70 p-4" onClick={onClose}>
      <form
        onSubmit={save}
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-md overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-2xl text-zinc-900 dark:text-zinc-100"
      >
        <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 px-5 py-4">
          <h3 className="font-black text-zinc-900 dark:text-white">Add an MCP server</h3>
          <button type="button" onClick={onClose} aria-label="Close"
                  className="rounded-lg p-1.5 text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white cursor-pointer">
            <X className="h-4 w-4" />
          </button>
        ...
        <div className="space-y-3 p-5">
          {[
            ...
            <label key={f.label} className="block">
              <span className="mb-1.5 block text-[11px] font-bold text-zinc-600 dark:text-zinc-400">{f.label}</span>
              <input
                value={f.value}
                required={f.required}
                placeholder={f.placeholder}
                onChange={(e) => f.set(e.target.value)}
                className="w-full rounded-xl border border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-black/40 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100
                           outline-none placeholder:text-zinc-400 dark:placeholder:text-zinc-600 focus:border-indigo-400/60 font-mono text-xs"
              />
            </label>
          ))}

          {error && (
            <p className="flex items-start gap-2 rounded-xl border border-amber-300 dark:border-amber-500/25 bg-amber-50 dark:bg-amber-500/10 px-3 py-2 text-[11px] text-amber-700 dark:text-amber-200">
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 py-2.5 text-sm
                       font-black text-white transition hover:bg-indigo-500 disabled:opacity-60 shadow-md cursor-pointer"
          >
