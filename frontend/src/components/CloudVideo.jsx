...
export default function CloudVideo({ onOpenModelHub }) {
  ...
  useEffect(() => {
    fetchWithAuth(`${API_BASE}/api/video/cloud`)
  ...
  const watch = useCallback((id) => {
    const tick = async () => {
      try {
        const res = await fetchWithAuth(`${API_BASE}/api/video/job/${id}`);
        const record = await res.json();
        setJob({ ...record, id });
        if (record.status === 'running') timer.current = window.setTimeout(tick, POLL_MS);
        else if (record.status !== 'completed') setError(record.error || 'The video was not made.');
  ...
  const start = async (event) => {
    ...
    try {
      const res = await fetchWithAuth(`${API_BASE}/api/video/cloud/generate`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: prompt.trim(), model, duration: Number(duration), aspect_ratio: aspect }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(typeof data.detail === 'string' ? data.detail : 'The video could not be started.');
      watch(data.job_id);
    } catch (err) { setJob(null); setError(err.message); }
  };

  const stop = () => job?.id && fetchWithAuth(`${API_BASE}/api/video/job/${job.id}/stop`, { method: 'POST' }).catch(() => {});
  ...
  if (!info.configured) {
    return (
      <div className={`${card} space-y-2 p-4 text-sm`}>
        <p className="font-bold text-zinc-900 dark:text-white">Cloud video needs a Replicate key</p>
        <p className="text-zinc-500">One key reaches Kling, Hailuo, Seedance, Wan, LTX and the other video models Replicate hosts. It is billed to your Replicate account, and the prompt goes to Replicate.</p>
      ...
      <form onSubmit={start} className={`${card} space-y-3 p-4 sm:p-5`}>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="flex items-end">
            {running ? (
              <button type="button" onClick={stop} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-rose-500/50 px-4 py-2 text-sm font-bold text-rose-500">
        ...
        <p className="flex items-center gap-1.5 text-[11px] text-zinc-500"><Cloud className="h-3.5 w-3.5" /> Length and shape are sent only to models that take them. Billed to your Replicate account.</p>
      </form>
      ...
      {job && (
        <div className={`${card} space-y-2 p-4`}>
          ...
          {job.status === 'completed' && (
            <>
              <video controls src={`${API_BASE}/api/video/file/${job.id}`} className="w-full rounded-xl bg-black" />
              <a href={`${API_BASE}/api/video/file/${job.id}`} download={`smaran-${job.id}.mp4`} className="inline-flex items-center gap-1.5 text-xs font-bold text-indigo-500">
