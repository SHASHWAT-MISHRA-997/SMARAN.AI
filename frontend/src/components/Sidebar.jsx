...
import {
  ...
  LogIn, Blocks, FolderOpen, Globe2, Compass, ArrowDownToLine, Terminal, Users,
...
const Sidebar = ({
  ...
  activeSection = 'code',
  onSectionChange,
  // `onMoveSession` is still passed by App but no longer read: the move-section
  // arrow has been taken off the conversation rows in both the desktop list and
  // the mobile drawer.
}) => {
  ...
  const sidebarDesktop = (
    ...
    `}>
      ...
      {/* SECTION SWITCHER: SMARAN Chat vs SMARAN Code */}
      <div className={`shrink-0 ${expanded ? 'px-2 pb-2 pt-1' : 'flex flex-col items-center gap-1.5 px-2 pb-2 pt-1'}`}>
        {expanded ? (
          <div className="flex items-center p-1 rounded-xl bg-zinc-900/90 dark:bg-zinc-900/90 border border-zinc-700/80 dark:border-zinc-700/80 shadow-inner">
            <button
              type="button"
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                activeSection === 'chat'
                  ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/40 border border-indigo-400/50 font-extrabold'
                  : 'text-zinc-300 hover:text-white hover:bg-zinc-800/60 border border-transparent'
              }`}
            >
              <MessageSquare className={`w-3.5 h-3.5 ${activeSection === 'chat' ? 'text-white' : 'text-indigo-400'}`} />
              <span className={activeSection === 'chat' ? 'text-white' : 'text-zinc-300'}>Chat</span>
            </button>
            <button
              type="button"
            ...
            >
              <Terminal className={`w-3.5 h-3.5 ${activeSection === 'code' ? 'text-white' : 'text-emerald-400'}`} />
              <span className={activeSection === 'code' ? 'text-white' : 'text-zinc-300'}>Code</span>
            </button>
          </div>
        ) : (
          <>
      ...
      {/* Destinations and conversations scroll together.

          They used to be two blocks: the destinations fixed with `shrink-0`,
          the conversations `flex-1` underneath. The destinations always took
          their full height first, so whatever was left went to the list -
          measured at 245px on a 720px screen and 125px on a 600px one, about
          two conversations out of forty. The list was scrollable the whole
          time; there was just so little of it that it did not look like it,
          which is why this was reported as "does not scroll".

          Sharing one scroller means a short window costs the nav some room
          rather than taking it all from the conversations. */}
      ...
      <div className={`shrink-0 ${expanded ? 'px-2 pb-2 space-y-0.5' : 'px-2 py-2 flex flex-col items-center gap-1'}`}>
        {expanded ? <>
          ...
          <button onClick={() => onNavigate('browser')} className="nav-neon sheen w-full flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200/70 dark:hover:bg-zinc-800/70 hover:text-zinc-950 dark:hover:text-white"><Compass className="h-4 w-4"/> Live Browser</button>
          ...
          <RailBtn icon={<Compass className="h-5 w-5"/>} label="Live Browser" onClick={() => onNavigate('browser')}/>
          {!handheld && <RailBtn icon={<Clock className="h-5 w-5"/>} label="Scheduled" onClick={() => onNavigate('scheduled')}/>}
        </>}
      </div>

      {/* Chat history. Scrolls with the destinations above, not against them. */}
      <div className={`${expanded ? 'px-3 py-1' : 'px-2 py-1 flex flex-col items-center gap-1'}`}>
        {expanded && (
      ...
      <div className={`relative shrink-0 border-t border-zinc-200 dark:border-zinc-800/80 p-2.5 ${expanded ? 'space-y-2' : 'flex flex-col items-center gap-1.5'}`}>
        {expanded ? (
          <>
            ...
            {/* Design now sits with the other destinations, under Sites, rather
                than alone down here below Settings and Clear. */}
    ...
    <>
      ...
      >
        ...
        <nav className="px-3 pb-3 space-y-1 border-b border-zinc-200 dark:border-zinc-800">
          ...
          <button onClick={() => { onNavigate('browser'); setMobileOpen(false); }} className={`w-full flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-xs font-black ${activeView === 'browser' ? 'bg-indigo-500/15 text-indigo-700 dark:text-indigo-300' : 'text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}><Compass className="h-4 w-4"/> Live Browser</button>
